...
then apply it to any connection to that model instead of rebuilding it by hand every time.
Distinct from <b>File &gt; Save/Load Session</b>: a session also carries this connection's own
IP/port/serial settings, a profile deliberately doesn't, since the same device model gets reused
across many different physical connections.</p>
...
configured Tags to capture into the profile, and a placeholder <b>Datasheet</b> page. The
...
means the group is a mix of both. Check the ones you want and click <b>Apply</b> to import them
into your live Tags list; Apply doesn't close the dialog, so you can adjust the selection and
...
<h3>Navigating</h3>
<p>The <b>Auto Scroll</b> checkbox (in the View Range group, next to Zoom In/Out) shows and
...
its own connection, Address Table, Profiles, Tags, Raw Data, Trend, Server, Script, and Scanner
tab. Use
