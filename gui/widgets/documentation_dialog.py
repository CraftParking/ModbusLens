...
adding things only at the end). Once a list is built, drag a row by its handle and drop it
elsewhere in the table to reorder it - the row's live values, comment, and alarm configuration
(if any) all move with it. Reordering is only available while monitoring is stopped, the same as
adding or removing a tag. Each row has:</p>
<ul>
