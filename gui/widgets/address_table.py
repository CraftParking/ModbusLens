...
class AddressTableWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.count_input.setValue(20)  # Default to 20 for new layout
        ...
        # Initially disable monitoring controls
    ...
    def create_address_table(self):
        ...
        for i in range(count):
            ...
            # Value column (editable for write operations, read-only for read operations)
            value_item = QTableWidgetItem("")
            if is_write:
                value_item.setFlags(value_item.flags() | Qt.ItemIsEditable)
                value_item.setBackground(QColor("#FFF8DC"))  # Light yellow for writable
            else:
                value_item.setFlags(value_item.flags() & ~Qt.ItemIsEditable)
    ...
    def on_cell_changed(self, row, column):
        """Handle cell value changes for write operations."""
        ...
        try:
            # Parse value based on function type
            if "Coils" in self.current_function:
            ...
            if success:
                ...
                self.log(f"Write failed to address {address}: {self.parent_window.modbus.last_error}")
    ...
    def write_value_to_device(self, address, value):
        ...
        try:
            if "Write Single Coil" in self.current_function:
                ...
                self.log("Error: Single write operation not supported for this function")
                return False
        except Exception as e:
